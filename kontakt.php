...
<p id="submitMessage" class="<?= $buttonClass ?>"><?= $formMessage ?><?php 
		...
		<div class="row">
				...
				<div class="col-sm-4 <?php if ($fehler["vorname"] != "") { echo 'error'; } ?> <?php echo (isset($_POST['vorname']) && ''!=$_POST['vorname'] ? 'not-empty-field ' : ''); ?>">
					...
					<input <?php if($cfg['HTML5_FEHLERMELDUNGEN']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Vorname" type="text" name="vorname" class="field"  placeholder="Vorname *" value="<?php echo $_POST['vorname']; ?>" maxlength="<?php echo $zeichenlaenge_vorname; ?>" id="border-right4" onclick="setActive(this);" onfocus="setActive(this);"/>
				...
				<div class="col-sm-4 <?php if ($fehler["name"] != "") { echo 'error'; } ?> <?php echo (isset($_POST['name']) && ''!=$_POST['name'] ? 'not-empty-field ' : ''); ?>">
					...
					<input <?php if($cfg['HTML5_FEHLERMELDUNGEN']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> type="text" name="name" class="field" placeholder="Nachname *" value="<?php echo $_POST['name']; ?>" maxlength="<?php echo $zeichenlaenge_name; ?>" id="border-right5" onclick="setActive(this);" onfocus="setActive(this);"/>
		   ...
		   <div class="row">
				<div class="col-sm-8 <?php if ($fehler["betreff"] != "") { echo 'error'; } ?> <?php echo (isset($_POST['betreff']) && ''!=$_POST['betreff'] ? 'not-empty-field ' : ''); ?>">
					...
					<input <?php if($cfg['HTML5_FEHLERMELDUNGEN']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Betreff" type="text" name="betreff" class="field" placeholder="Betreff *" value="<?php echo $_POST['betreff']; ?>" maxlength="<?php echo $zeichenlaenge_betreff; ?>" id="border-right8" onclick="setActive(this);" onfocus="setActive(this);"/>
			...
			<div class="row"> 
				<div class="col-sm-8 <?php if ($fehler["nachricht"] != "") { echo 'error'; } ?> <?php echo (isset($_POST['nachricht']) && ''!=$_POST['nachricht'] ? 'not-empty-field ' : ''); ?>">
					...
					<textarea <?php if($cfg['HTML5_FEHLERMELDUNGEN']) { ?> required <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Nachricht" name="nachricht" class="field" rows="5" placeholder="Nachricht *" style="height:100%;width:100%;" id="border-right9" onclick="setActive(this);" onfocus="setActive(this);"><?php echo $_POST['nachricht']; ?></textarea>
		...
		// -------------------- DATEIUPLOAD START ----------------------
			if(0<$cfg['NUM_ATTACHMENT_FIELDS']){
				...
				for ($i=0; $i < $cfg['NUM_ATTACHMENT_FIELDS']; $i++) {
							echo '<input aria-label="Dateiupload" type="file" size=12 name="f[]" id="upload_field" style="font-size:17px;" onclick="setActive(this);" onfocus="setActive(this);"/>';
				}
				echo '	</div>
					</div>';
			}
		...
		if($cfg['Sicherheitscode']) { ?>
			<div class="row captcha-row <?php if ($fehler["captcha"] != "") { echo 'error_container'; } ?>" style="background-position: 2.85rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
				<div class="col-sm-8 <?php if ($fehler["captcha"] != "") { echo 'error'; } ?>">
					...
					<input <?php if($cfg['HTML5_FEHLERMELDUNGEN']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Eingabe" id="answer2" placeholder="Sicherheitscode *" type="text" name="sicherheitscode" maxlength="150"  class="field<?php if ($fehler["captcha"] != "") { echo ' errordesignfields'; } ?>" onclick="setActive(this);" onfocus="setActive(this);" />
		...
		if($cfg['Sicherheitsfrage']) { ?>
			...
			<div class="row question-row <?php if ($fehler["q_id12"] != "") { echo 'error_container'; } ?>" style="background-position: 2.85rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
				<div class="col-sm-8 <?php if ($fehler["q_id12"] != "") { echo 'error'; } ?>">
					...
					<input <?php if($cfg['HTML5_FEHLERMELDUNGEN']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Antwort" id="answer" placeholder="Antwort *" type="text" class="field<?php if ($fehler["q_id12"] != "") { echo ' errordesignfields'; } ?>" name="q" onclick="setActive(this);" onfocus="setActive(this);" />
		...
		if($cfg['Datenschutz_Erklaerung']) { ?>
			<div class="row checkbox-row <?php if ($fehler["datenschutz"] != "") { echo 'error_container'; } ?>" style="background-position: 2.85rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
...
<div style="border: 1px solid #ccc; margin: 0;box-sizing: border-box;display: inline-block;line-height: 0.3;vertical-align: top;padding-bottom:7.5px;padding-right:5px;padding-left:5px;">
						<br /><br /><a href="https://www.kontaktformular.com" title="kontaktformular.com" style="text-decoration: none;color:#000000;font-size:13.5px;line-height: 20px;" target="_blank">&copy; by kontaktformular.com - Alle Rechte vorbehalten.</a>
</div>
</div>
<!-- Dieser Copyrighthinweis darf NICHT entfernt werden. -->
